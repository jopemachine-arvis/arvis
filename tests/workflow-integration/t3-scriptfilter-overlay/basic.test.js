...
describe('Integration test', function () {
  ...
  it('Test 3.2 - scriptFilter + scriptFilter, popwork', async function () {
    await app.client.waitUntilWindowLoaded();
    await app.client.switchWindow('searchWindow');
    ...
    const itemTitle4 = await (
      await app.client.$('#searchResultItemTitle-0')
    ).getText();
    const itemSubTitle4 = await (
      await app.client.$('#searchResultItemSubTitle-0')
    ).getText();

    assert.strictEqual(itemTitle4, 'scriptfilter item 1');
    assert.strictEqual(itemSubTitle4, 'subtitle 1');

    await sleep(500);

    robot.keyTap('escape');

    const itemTitle5 = await (
      await app.client.$('#searchResultItemTitle-0')
    ).getText();
    const itemSubTitle5 = await (
      await app.client.$('#searchResultItemSubTitle-0')
    ).getText();
